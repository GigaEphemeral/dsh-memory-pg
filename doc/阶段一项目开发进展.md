...
> 提示解除。
> `2026-09-14`（第四次）— **任务拆分**：新建 [`阶段一task.md`](阶段一task.md) 收纳详细待办（逐项验收
> 与备注）；README §5/§6 改为概览并链接 阶段一task.md；新增 §17 Future（暂时不做）清单。
...
> **详细待办已移入 [`阶段一task.md`](阶段一task.md)**（按里程碑逐项列出、含验收与备注）；本节只留
> 功能一览表，供快速定位。
...
> **详细任务（逐项、含验收与备注）已移入 [`阶段一task.md`](阶段一task.md)**；本节只留里程碑概览。
...
> 每个里程碑的**退出标准**见 [`阶段一task.md`](阶段一task.md) 对应小节。
...
| `~/.dsh` | 存在：`C:\Users\***\.dsh`（目录） | `Test-Path` |
| 运行中的 DSH web | `http://127.0.0.1:3080`（本会话即用它，**不可污染**） | — |
| **PostgreSQL（Docker）** | `localhost:54320`，库 `postgres`，用户 `postgres`，密码 `***` | TCP 54320 OPEN；`docker ps` → `dsh_memory`（`dawsonlp/postgres-batteries-inc:latest`，healthy） |
...
✅ 本机已有现成容器（§8.0）：`dsh_memory`（`localhost:54320`，`postgres`/`***`，
...
- **方式一（推荐，本机现成）**：用 `dsh_memory` 容器，**新建独立 database**：
  ...
  # 连接串: postgres://postgres:***@localhost:54320/dsh_memory_pg_test
...
2. ✅ **M0 技术验证已完成**（2026-09-14）：命令 kebab-case 注册、D4=路线 A、PG+pgvector+HNSW、
   D7 方案、隔离实例——详见 [`阶段一task.md`](阶段一task.md) M0
...
> 详细登记见 [`阶段一task.md`](阶段一task.md) 的 Future 小节。
