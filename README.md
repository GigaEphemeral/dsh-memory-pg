...
#### 4.4.1 硬编码参数与决策依据（2026-09-14 补充）

提炼与入库管线当前的**硬编码参数**统一登记于此；多数是 **b1 求稳妥的临时值**，后续里程碑
应参数化（进设置面板或 config）：

| 参数 | 值 | 位置 | 决策依据 / 状态 |
|---|---|---|---|
| 取会话上下文条数 | **40 条**（最近 40 条） | `src/index.ts` contextProvider | ⚠️ **b1 稳妥决策**：防止一次性喂太多上下文、也够覆盖近期对话；**无正式方案权衡**，是快速出 b1 的临时上限，待参数化（用户 2026-09-14 指出未在 README 写明） |
| 长上下文分块 | `chunkChars=1000`，`maxChunks=6` | `src/distill.ts` | 参照 §16.2（参考实现 splitTranscript 同值）；可配 |
| 提炼事实上限 | `maxFacts=8` | `src/distill.ts` | 一次命令最多 8 条，避免命令结果过长；可配 |
| 单条过长阈值 | `maxChars=600` | `src/segment.ts` | §3.3（超过递归分割兜底）；可配 |
| 单条过短阈值 | `minChars=20` | `src/segment.ts` | §3.3（低于与相邻合并）；可配 |
| 近似去重阈值 | `dedupScore=0.92` | `src/segment.ts` | §3.4（参考实现验证过的初值） |
| 冲突检测阈值 | `conflictScore=0.86` | `src/segment.ts` | §3.4（参考实现验证过的初值） |

> 原则：**去重/冲突阈值**有参考实现背书（§3.4）；**条数/分块/上限**是 b1 稳妥值，待 M5
> 设置面板参数化时评审。所有值已在 `src/*.ts` 的 `*_DEFAULTS` 常量集中定义，便于迁移到配置。

> ✅ D4 已定（M0，2026-09-14）：走**路线 A**（`ctx.llm.stream()`）——`ctx.llm` 挂载 +
> `llm-deepseek` 适配器就绪，实际冒烟并入后续里程碑。
...
| — | 多用户/多租户权限、多人协作、独立 Web 管理后台 | v2（用户 2026-09-14 确认从 §1 非目标移入 v2） | — |
| — | 省 token 规划：参照官方前缀缓存复用 | backlog（待调研确认方案） | 手动触发场景缓存收益不确定；方案调研后定（2026-09-14 用户指示） |
